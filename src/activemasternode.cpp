...
//
// Bootup the Masternode, look for a 1000 DFS input and register on the network
//
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (service.GetPort() != Params().GetDefaultPort()) {
                notCapableReason = strprintf("Invalid port: %u - only %d is supported on mainnet.", service.GetPort(),Params().GetDefaultPort());
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
                return;
            }
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (service.GetPort() != Params().GetDefaultPort()) {
            errorMessage = strprintf("Invalid port %u for masternode %s - only %d is supported on mainnet.", service.GetPort(), strService,Params().GetDefaultPort());
            LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
            return false;
        }
    } else if (service.GetPort() == Params().GetDefaultPort()) {
